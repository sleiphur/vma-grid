...
const Address = {
  isRangeRef: (param: any) => param.ref && param.ref.from,

  isCellRef: (param: any) => param.ref && !param.ref.from,

  columnNumberToName: (number: any) => {
    let dividend = number
    let name = ''
    let modulo = 0
   ...
   * @param {{ref: {}}} [range2]
   */
  extend: (range1: any, range2: any) => {
    if (range2 == null) {
      return range1
    }
    let rowOffset
    let colOffset
    if (Address.isCellRef(range1)) {
      rowOffset = 0
      colOffset = 0
    } else if (Address.isRangeRef(range1)) {
...
class FormulaHelpers {
  get Types(): Record<string, unknown> {
    return this._Types
  }

  set Types(value: Record<string, unknown>) {
    this._Types = value
  }

  private _Types: Record<string, unknown>

  private readonly type2Number: Record<string, unknown>
  ...
  checkFunctionResult = (result: any) => {
   ...
   */
  acceptNumber(obj: any, isArray = true, allowBoolean = true): any {
    // check error
   ...
   * @param {_Types|null} valueType - The type each item should be,
   ...
   * @param range2
   */
  retrieveRanges(context: any, range1: any, range2: any) {
    // process args
    range2 = Address.extend(range1, range2)

    // retrieve values
    range1 = this.retrieveArg(context, range1)
    range1 = this.accept(range1, Types.ARRAY, undefined, false, true)

    if (range2 !== range1) {
      range2 = this.retrieveArg(context, range2)
      range2 = this.accept(range2, Types.ARRAY, undefined, false, true)
    } else range2 = range1

    return [range1, range2]
  }

  retrieveArg = (context: any, arg: any) => {
...
const WildCard = {
  ...
  toRegex: (lookupText: any, flags?: any) =>
    RegExp(
      lookupText
