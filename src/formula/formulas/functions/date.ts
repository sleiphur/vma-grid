...
function parseSimplifiedDate(text: any) {
...
function parseDateWithExtra(serialOrString: any) {
...
function parseDate(serialOrString: any) {
  return parseDateWithExtra(serialOrString).date
}

function compareDateIgnoreTime(date1: any, date2: any) {
  return (
...
const DateFunctions = {
  ...
  DATEDIF: (startDate: any, endDate: any, unit: any) => {
  ...
  DAY: (serialOrString: any) => {
    const date = parseDate(serialOrString)
    return date.getUTCDate()
  },

  DAYS: (endDate: any, startDate: any) => {
    endDate = parseDate(endDate)
    startDate = parseDate(startDate)
    let offset = 0
  ...
  DAYS360: (startDate: any, endDate: any, method: any) => {
    startDate = parseDate(startDate)
    endDate = parseDate(endDate)
  ...
  EDATE: (startDate: any, months: any) => {
    startDate = parseDate(startDate)
  ...
  EOMONTH: (startDate: any, months: any) => {
    startDate = parseDate(startDate)
    months = H.accept(months, Types.NUMBER)
  ...
  HOUR: (serialOrString: any) => {
    const date = parseDate(serialOrString)
    return date.getUTCHours()
  },

  ISOWEEKNUM: (serialOrString: any) => {
    const date = parseDate(serialOrString)
  ...
  MINUTE: (serialOrString: any) => {
    const date = parseDate(serialOrString)
    return date.getUTCMinutes()
  },

  MONTH: (serialOrString: any) => {
    const date = parseDate(serialOrString)
    return date.getUTCMonth() + 1
  },

  NETWORKDAYS: (startDate: any, endDate: any, holidays: any) => {
    startDate = parseDate(startDate)
    endDate = parseDate(endDate)
    let sign = 1
  ...
  WEEKDAY: (serialOrString: any, returnType: any) => {
    const date = parseDate(serialOrString)
  ...
  WEEKNUM: (serialOrString: any, returnType: any) => {
    const date: any = parseDate(serialOrString)
    returnType = H.accept(returnType, Types.NUMBER, 1)
    if (returnType === 21) {
      return DateFunctions.ISOWEEKNUM(serialOrString)
    }
    const weekStart: any = WEEK_STARTS[returnType]
    const yearStart = new Date(Date.UTC(date.getUTCFullYear(), 0, 1))
    const offset = yearStart.getUTCDay() < weekStart ? 1 : 0
    return Math.ceil(((date - yearStart.getTime()) / 86400000 + 1) / 7) + offset
  },

  WORKDAY: (startDate: any, days: any, holidays: any) =>
    DateFunctions['WORKDAY.INTL'](startDate, days, 1, holidays),

  'WORKDAY.INTL': (startDate: any, days: any, weekend: any, holidays: any) => {
    startDate = parseDate(startDate)
  ...
  YEAR: (serialOrString: any) => {
    const date = parseDate(serialOrString)
    return date.getUTCFullYear()
  },

  // Warning: may have bugs
  YEARFRAC: (startDate: any, endDate: any, basis: any) => {
    startDate = parseDate(startDate)
    endDate = parseDate(endDate)
    if (startDate > endDate) {
      const temp = startDate
