...
const ReferenceFunctions = {
  ...
  HLOOKUP: (lookupValue: any, tableArray: any, rowIndexNum: any, rangeLookup: any) => {
    ...
    if (WildCard.isWildCard(lookupValue)) {
      ...
      index = tableArray[0].findIndex((item: any) => item === lookupValue)
    }
  ...
  VLOOKUP: (lookupValue: any, tableArray: any, colIndexNum: any, rangeLookup: any) => {
    ...
    let index = -1
    if (WildCard.isWildCard(lookupValue)) {
      index = tableArray.findIndex((currRow: any) =>
        WildCard.toRegex(lookupValue, 'i').test(currRow[0]),
      )
    } else {
      index = tableArray.findIndex((currRow: any) => currRow[0] === lookupValue)
    }
    // the exact match is not found
    if (index === -1) throw FormulaError.NA
