...
class Utils {
  ...
  parseColRange(col1: any, col2: any) {
  ...
  parseRowRange = (row1: any, row2: any) =>
  ...
  _applyPrefix(prefixes: any, val: any, isArray: any) {
    if (this.isFormulaError(val)) return val
    return Prefix.unaryOp(prefixes, val, isArray)
  }

  async applyPrefixAsync(prefixes: any, value: any) {
  ...
  _applyPostfix(val: any, isArray: any, postfix: any) {
    if (this.isFormulaError(val)) return val
    return Postfix.percentOp(val, postfix, isArray)
  }

  async applyPostfixAsync(value: any, postfix: any) {
    const { val, isArray } = this.extractRefValue(await value)
  ...
  _applyInfix(res1: any, infix: any, res2: any) {
    const val1 = res1.val
  ...
  async applyInfixAsync(value1: any, infix: any, value2: any) {
  ...
  static formatChevrotainError(error: any, inputText: any) {
    let line
    let column
    let msg = ''
    // e.g. SUM(1))
